...
public class ElimNot_FOL extends FOLRule {
	...
	public FOLViolation check(FOLSequent conclusion, FOLSequent... premises) {
		...
		Set<FOLFormula> gamma0 = s0.antecedent(), gamma1 = s1.antecedent(), gamma = conclusion.antecedent();
		...
		if (!gamma.equals(gamma1))
			return violation(conclusion, premises, fill("The second premise's antecedent, " + gamma1
					+ ", and the conclusion's antecedent, " + gamma + ", should be equal, but are not."));
		FOLFormula a = s0.succedent(), not = s1.succedent();
	...
	public void printDescription(PrintStream out) {
		...
		String s1 = GAMMA + " " + TextUtil.infers() + " A     " + GAMMA + " " + TextUtil.infers() + " " + TextUtil.not() + "A";
		String s2 = GAMMA + " " + TextUtil.infers() + " B";
		TextUtil.printFrac(out, 5, s1, s2);
		StringBuilder buf = new StringBuilder();
		buf.append("Rule " + this + " says that if in some context " + GAMMA + ", you can dervive both A and " + TextUtil.not()
	...
	public boolean equals(Object obj) {
		return obj instanceof ElimNot_FOL;
	}

	@Override
	public int hashCode() {
		return ElimNot_FOL.class.hashCode();
	}
}
