...
/**
 * Represents the natural deduction inference rule ∀-Introduction (IntroForall)
 * for First-Order Logic (FOL).
 * <p>
 * This rule allows the generalization of a conclusion from a specific instance
...
public class IntroForall extends FOLRule {
	 ...
	 * ∀-Introduction rule. The rule states: From Γ ⊢ φ[y/x], conclude Γ ⊢ ∀x φ,
	 * where y is free for x in φ, and y does not occur free in Γ or φ. This method
	 * performs the following validations:
	 * <ol>
	 * <li>Antecedent Check: Γ must match</li>
	 * <li>Conclusion must be a universal formula (∀x φ)</li>
	 * <li>Premise must be a substitution instance φ[y/x] using some variable y</li>
	 * <li>y must be free for x in φ</li>
	 * <li>y must not occur free in Γ</li>
	 * <li>y must not occur free in φ (if y != x)</li>
	 * </ol>
	 *
	...
	public FOLViolation check(FOLSequent conclusion, FOLSequent... premises) {
		...
		// 3. Substitution Check: Is premise a substitution instance A[y/x] using some
		// variable y.?
		Term t = fac.findSubstitutionTerm(body, x, f_prem);
		if (t == null || !(t instanceof Variable)) {
			return violation(conclusion, premises,
					fill("The premise " + f_prem + " must be a substitution instance using a variable y."));
		}
		...
		if (!fac.isFreeFor(y, x, body)) {
			return violation(conclusion, premises,
					fill("Side condition 1: " + y + " is not free for " + x + " in " + body));
		}
		// Side condition 2: y does not occur free in Γ (assumptions).
		for (FOLFormula ass : gamma) {
			if (fac.freeVars(ass).contains(y)) {
				return violation(conclusion, premises,
						fill("Side condition 2: " + y + " must not occur free in the assumptions: " + ass));
			}
		}
		// Side condition 2: y does not occur free in φ (the body).
		// Note: If y != x, we must ensure y isn't already free in φ before
		// substitution.
		if (y != x && fac.freeVars(body).contains(y)) {
			return violation(conclusion, premises,
					fill("Side condition 2: " + y + " must not occur free in the body of the conclusion" + body));
		}
		return null;
	}
