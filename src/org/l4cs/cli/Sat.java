...
public class Sat {
	...
	public static void execute(CommandLine cl) {
		...
		if (cl.model) {
			Model model = solver.sat(f);
			if (model == null)
				out.println("unsat");
			else
				out.println(model);
		} else {
			out.println(solver.isSat(f) ? "true" : "false");
		}
	}

	public static void describe(CommandLine cl) {
		out.println("Usage: lap sat <options> [<filename>]");
		out.println("Description:");
		out.println("  Determines whether a propositional formula is satisfiable.");
		...
		out.println("    brute : brute force, iterating over all models until a satisfying");
		out.println("            model is found.");
		out.println("    dpll  : the formula is first converted to an equisatisfiable ");
